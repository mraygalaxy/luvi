...
if [ $? -eq 0 ] ; then 
	if [ x"$1" == xslave ] ; then
		...
		~/mythtv_to_dvd/luvi/luvi_slave $2 1264
	else
		#gdb ~/mythtv_to_dvd/luvi/luvi_master -ex "run \"${file}\" \"$video\" 1264 1"
		#~/mythtv_to_dvd/luvi/luvi_master "${file}" "$video" 1264 0 # build index
		#~/mythtv_to_dvd/luvi/luvi_master "${file}" "$video" 1264 0 # local transcode
		~/mythtv_to_dvd/luvi/luvi_master "${file}" "$video" 1264 1 # slave transcode
	fi
fi
